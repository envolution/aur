...
def fetch_aur_data(maintainer):
    ...
    try:
        ...
        if data.get("type") == "error": aur_logger.error(f"AUR API error for '{maintainer}': {data.get('error')}"); return aur_data_by_pkgbase
        if data.get("resultcount", 0) == 0: aur_logger.info(f"No packages found on AUR for maintainer '{maintainer}'."); return aur_data_by_pkgbase
        
        count = 0
        for result in data.get("results", []):
            name, base_name, full_ver = result["Name"], result["PackageBase"], result["Version"]
    ...
    except subprocess.CalledProcessError as e: aur_logger.error(f"AUR query fail for '{maintainer}' (code {e.returncode}): {e.stderr}")
    except json.JSONDecodeError as e: aur_logger.error(f"Failed to parse AUR JSON for '{maintainer}': {e}")
    except Exception as e: aur_logger.error(f"AUR fetch error for '{maintainer}': {e}", exc_info=aur_logger.isEnabledFor(logging.DEBUG))
...
def fetch_local_pkgbuild_data(path_root, pkgbuild_script_path):
    ...
    if not os.path.exists(actual_script_path): local_logger.critical(f"Script '{actual_script_path}' not found."); return local_data_by_pkgbase
    abs_path_root = os.path.abspath(path_root)
    pkgbuild_glob = os.path.join(abs_path_root, '**', 'PKGBUILD')
    local_logger.info(f"Searching PKGBUILDs in '{abs_path_root}' using '{pkgbuild_glob}'")
    pkg_files = glob.glob(pkgbuild_glob, recursive=True)
    if not pkg_files: local_logger.warning(f"No PKGBUILDs found in '{abs_path_root}'."); return local_data_by_pkgbase
    ...
    try:
        ...
        for item in parsed_items:
            json_pkgbase, json_pkgname, pkgfile = item.get("pkgbase"), item.get("pkgname"), item.get("pkgfile")
            key_pkgbase = json_pkgbase
            if not key_pkgbase and pkgfile: key_pkgbase = os.path.basename(os.path.dirname(pkgfile))
            if not key_pkgbase: local_logger.warning(f"Cannot determine pkgbase for item: {item}. Skipping."); continue
    ...
    except subprocess.TimeoutExpired: local_logger.error(f"Call to '{actual_script_path}' timed out.")
    except json.JSONDecodeError as e: local_logger.error(f"Failed to parse JSON from '{actual_script_path}': {e}\nData: {res_json[:500]}...")
    except Exception as e: local_logger.error(f"Local PKGBUILD fetch error: {e}", exc_info=local_logger.isEnabledFor(logging.DEBUG))
    return local_data_by_pkgbase
...
def run_nvchecker(path_root, oldver_data_for_nvchecker, key_toml_path_arg):
    ...
    if not toml_files: nv_logger.warning(f"No .nvchecker.toml in '{abs_path_root}'. Skipping NVChecker."); return results_by_pkgbase
    ...
    with tempfile.TemporaryDirectory(prefix="aurupdater_nv_") as tmpdir:
        all_nv_tomls_path, oldver_json_path, newver_json_path = (os.path.join(tmpdir, f) for f in ["all_nv.toml", "oldver.json", "newver.json"])
        key_file_to_use = None
        if key_toml_path_arg:
            abs_user_key = os.path.abspath(key_toml_path_arg)
            if os.path.exists(abs_user_key): key_file_to_use = abs_user_key; nv_logger.info(f"Using user key file: {key_file_to_use}")
            else: nv_logger.warning(f"User key file '{abs_user_key}' not found.")
        else:
            gh_token = os.environ.get("GITHUB_TOKEN")
            if gh_token:
                temp_key_file = os.path.join(tmpdir, "env_gh_key.toml")
                try:
                    with open(temp_key_file, "w") as f: f.write(f"[keys]\ngithub = '{gh_token}'\n")
                    key_file_to_use = temp_key_file
                    nv_logger.info(f"Using temp key file from GITHUB_TOKEN: {key_file_to_use}")
                except Exception as e: nv_logger.error(f"Failed to write temp key file: {e}")
        ...
        for tf in toml_files:
            try:
                with open(tf, "r") as f: content.extend([f"# Source: {os.path.relpath(tf, abs_path_root)}\n", f.read(), "\n\n"])
            except Exception as e: nv_logger.error(f"Error reading {tf}: {e}"); continue
        with open(all_nv_tomls_path, "w") as f: f.write("".join(content))
        if nv_logger.isEnabledFor(logging.DEBUG): nv_logger.debug(f"Concatenated .nvchecker.toml (first 500c):\n{''.join(content)[:500]}...")
        with open(oldver_json_path, "w") as f: json.dump({"version": 2, "data": oldver_data_for_nvchecker}, f)
        ...
        if key_file_to_use: cmd.extend(['-k', key_file_to_use])
        
        nv_logger.info(f"Running NVChecker: \"{' '.join(cmd)}\" (in {tmpdir})")
        stdout_data, stderr_data, code = "", "", 1
        try:
            proc = subprocess.run(cmd, capture_output=True, text=True, cwd=tmpdir, check=False, timeout=180)
            stdout_data, stderr_data, code = proc.stdout, proc.stderr, proc.returncode
            if code != 0: nv_logger.error(f"NVChecker exited with code {code}.")
            if stderr_data: nv_logger.warning(f"NVChecker STDERR:\n{stderr_data.strip()}")
            if not stdout_data.strip() and code == 0: nv_logger.info("NVChecker ran successfully, no JSON output on STDOUT.")
        except FileNotFoundError: nv_logger.critical("nvchecker command not found."); return results_by_pkgbase
        except subprocess.TimeoutExpired: nv_logger.error("NVChecker command timed out."); return results_by_pkgbase
        except Exception as e: nv_logger.error(f"NVChecker execution error: {e}", exc_info=True); return results_by_pkgbase
        ...
        for line in stdout_data.strip().split('\n'):
            ...
            try:
                ...
                elif ev == "no-result": nv_logger.warning(f"NVCR: {name} NO-RESULT. {msg}")
                elif lvl == "error" or entry.get("exc_info"): nv_logger.warning(f"NVCR: {name} ERROR - {entry.get('exc_info',msg)}")
...
def process_and_compare_data(all_data_by_pkgbase):
    ...
    for pkgbase_key, data in all_data_by_pkgbase.items():
        ...
        display_name = data.get("aur_actual_pkgname") or data.get("local_actual_pkgname") or \
                       data.get("nvchecker_name_reported") or pkgbase_key 
        ...
        if not update_found and not pkg_entry['errors'] and local_base:
            ahead_aur = compare_package_versions(local_base, local_r, aur_base, aur_r) == "downgrade"
...
def generate_summary(processed_output_list, stream=sys.stderr):
    ...
    updates_nv, updates_aur, errors_list, nv_issues, up_to_date_count, local_ahead_list = [], [], [], [], 0, []
    ...
    for pkg_data in processed_output_list:
        ...
        if pkg_data['is_update']:
            ...
            local_full_ver = _get_full_version_string(pkg_data.get('pkgver'), pkg_data.get('pkgrel'))
            local_ahead_list.append(f"{id_name}{display_name_info} (Local: {local_full_ver})")
        ...
        if not pkg_data['is_update'] and not pkg_data['errors'] and not pkg_data.get('local_is_ahead'):
            ...
            if nv_raw_log:
                ...
                if nv_event == 'no-result' or nv_level == 'error' or nv_raw_log.get('exc_info'):
                    ...
                    nv_issues.append(f"{id_name}{display_name_info}: NVChecker operational issue - {details}")
    ...
    if local_ahead_list: lines.extend([f"\nLocal Packages Ahead of AUR/NVChecker ({len(local_ahead_list)}):"] + [f"  - {s}" for s in local_ahead_list])
    if not updates_nv and not updates_aur and not local_ahead_list and not errors_list : lines.append("\nNo updates, local ahead, or errors.")
    elif not updates_nv and not updates_aur and not local_ahead_list : lines.append("\nNo updates or local ahead (check errors).")
    if up_to_date_count > 0 : lines.append(f"\nPackages up-to-date or no other action: {up_to_date_count}")
    if errors_list: lines.extend([f"\nVersion Discrepancies ({len(errors_list)}):"] + [f"  - {s}" for s in errors_list])
    if nv_issues: lines.extend([f"\nNVChecker Operational Issues ({len(nv_issues)}):"] + [f"  - {s}" for s in nv_issues])
...
class AurPackageUpdater:
    ...
    def _configure_logging(self):
        ...
        logger.info(f"App logger '{logger.name}' effective level: {logging.getLevelName(logger.getEffectiveLevel())}")
    ...
    def run(self):
        ...
        if not os.path.isdir(self.args.path_root):
            logger.critical(f"Path root '{self.args.path_root}' is not valid. Exiting."); sys.exit(1)
        ...
        try:
            ...
            if self.args.output_file: output_stream.write("\n"); logger.info(f"JSON output written to {self.args.output_file}")
        finally:
            if self.args.output_file and output_stream is not sys.stdout: output_stream.close()
        
        if self.args.summary: generate_summary(final_output_list, stream=sys.stderr)

# --- CLI Argument Parsing and Main Execution ---
def main_cli():
    parser = argparse.ArgumentParser(description="AUR/local/NVChecker package update tool.", formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    ...
    parser.add_argument("--pkgbuild-script", default="./pkgbuild_to_json.py", help="Script to parse PKGBUILDs to JSON.")
    parser.add_argument("--key-toml", default=None, help="Path to NVChecker's key.toml. Checks GITHUB_TOKEN env var if not set.")
    parser.add_argument("--output-file", default=None, help="File for JSON output (default: STDOUT).")
    parser.add_argument("--summary", action="store_true", help="Print summary to STDERR.")
    parser.add_argument("--debug", action="store_true", help="Enable debug logging.")
    args = parser.parse_args()
    app = AurPackageUpdater(args)
    try: app.run()
    except Exception as e: logger.critical(f"Unhandled exception: {e}", exc_info=True); sys.exit(2)
