...
class ArchPackageBuilder:
    ...
    def _setup_logger(self) -> logging.Logger:
        ...
        # FIXME
        instance_logger.setLevel(logging.DEBUG) # Force DEBUG for testing
        instance_logger.info(f"Builder instance logger set to level: {logging.getLevelName(instance_logger.getEffectiveLevel())}")
        # /FIXME
...
# In buildscript2.py / ArchPackageBuilder class
    ...
    def build_package(self) -> bool:
        ...
        try:
            ...
            # Use simpler redirection to avoid hanging
            build_cmd = ["paru", "-Ui", "--noconfirm", "--mflags", "-Lfs --noconfirm --noprogressbar"]
            build_result = self.subprocess_runner.run_command(build_cmd)

            # Write output to log file
            try:
                with open(self.build_dir / "paru.log", "w") as log_file:
                    log_file.write(f"Command: {' '.join(build_cmd)}\n")
                    log_file.write(f"Return code: {build_result.returncode}\n")
                    log_file.write("STDOUT:\n")
                    log_file.write(build_result.stdout)
                    log_file.write("\nSTDERR:\n")
                    log_file.write(build_result.stderr)
            except Exception as e:
                self.logger.warning(f"Failed to write paru.log: {e}")
...
# In buildscript2.py / ArchPackageBuilder class
    ...
    def run(self) -> Dict[str, Any]:
        ...
        self.logger.warning(f"--- BUILD SCRIPT 2 RUN METHOD ENTERED FOR {self.config.package_name} ---") # Force a WARNING
