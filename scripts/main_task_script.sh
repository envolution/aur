...
generate_nvchecker_config() {
  ...
  echo "[keys]" > "${keyf}"; echo "github = '${SECRET_GHUK_VALUE}'" >> "${keyf}"
...
extract_pkgbuild_details() { 
  ...
  if [ ! -f "${pkgbuild_dir_abs}/PKGBUILD" ]; then echo "::error title=EXTRACT_FAIL::PKGBUILD missing in ${pkgbuild_dir_abs}" >&2; return 1; fi
