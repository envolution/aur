...
generate_nvchecker_config() {
  ...
  local cfg="new.toml"
  local keyf="keyfile.toml" # This file is created in NVCHECKER_RUN_DIR
  echo "[__config__]" >"${cfg}"
  echo "oldver = 'aur.json'" >>"${cfg}"
  echo "newver = 'local.json'" >>"${cfg}"
  echo "[keys]" >"${keyf}"
  echo "github = '${SECRET_GHUK_VALUE}'" >>"${keyf}" # SECRET_GHUK_VALUE should be an env var
  ...
  if [ ${#individual_configs[@]} -gt 0 ]; then
    ...
    for cf_item in "${individual_configs[@]}"; do # Renamed loop var
      cat "${cf_item}" >>"${cfg}"
      echo "" >>"${cfg}"
    done
  else _log_warning "NV_CONF" "No individual .nvchecker.toml files found."; fi
  
  # Ensure builder owns these files created in NVCHECKER_RUN_DIR
  if ! sudo -u builder chown builder:builder "${cfg}" "${keyf}"; then
