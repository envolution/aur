...
def parse_pkgbuild_output(output_str: str) -> dict:
        ...
        for internal_marker, json_key in current_key_map.items():
            if line == f"{internal_marker}_START":
                if active_key_internal:
                    print(f"Warning: Encountered new START marker '{line}' while '{active_key_internal}' was active.", file=sys.stderr)
    ...
    if active_key_internal and current_values:
        print(f"Warning: Data collection for '{active_key_internal}' might be incomplete (no END marker found).", file=sys.stderr)
        if active_key_json in ["pkgname", "pkgbase", "pkgver", "pkgrel"]:
            data[active_key_json] = current_values[0] if current_values else ""
        else:
            data[active_key_json] = [v for v in current_values if v]
...
def main():
    parser = argparse.ArgumentParser(description="Source PKGBUILD files and extract variables as JSON.")
    parser.add_argument(
        "pkgbuild_files",
        metavar="FILE",
        type=Path,
